...
.PHONY: all test clean deploy fund help install snapshot format anvil zktest deployAll
...
ifeq ($(findstring --network amoi,$(ARGS)),--network amoi)
	NETWORK_ARGS := --rpc-url $(AMOI_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif
